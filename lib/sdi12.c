...
uint8_t _txEnable; // (JMC: 0.10 reference to the pin that connects to one of the SN74HCT240 output enable pins)
uint8_t _txDataPin; // (JMC: 0.11 reference to the tx data pin)
uint8_t _rxEnable; // (JMC: 0.12 reference to the pin that connects to one of the SN74HCT240 output enable pins)
...
char _rxBuffer[_BUFFER_SIZE]; // 1.2 - declare an array (buffer for incoming ascii characters)
...
return;
 ...
 pullUpDnControl(_rxDataPin, PUD_UP);// Triggers the first interrupt which has a bug and triggers two. Both these need to be ignored.
 delay(1);
 digitalWrite(_rxEnable, HIGH); // State of 240 output 1 in high impedance
 digitalWrite(_txDataPin, HIGH); // Set TX pin HIGH level (txDataPin = BCM17)
 digitalWrite(_txEnable, LOW); // Set State of 240 output 2 'driving' state
...
bits, (LSB first) 1 start bit, 1 parity bit (even parity), and 1 stop bit. The SDI12
protocol uses negative logic. An example a transmission of character 'a' is
...
// 5.6 - reads in the next character from the buffer and moves the index ahead. (JMC: This is FIFO opperation)
int SDI12::read() {
 _bufferOverflow = false;
...
inline void SDI12::receiveChar() {
 ...
 delayMicroseconds(20); // 6.2.3 - sets a small delay period after the falling edge of the start bit was detected
 for (uint16_t i=0x1; i<=0x80; i <<= 1) { // 6.2.4 - read the 7 data bits (for i = 1 to s0100 0000 (<<= bitshift assignment))
...
delayMicroseconds(650); //(650) // 6.2.5 - Skip the stop bit.
 ...
 if (digitalRead(_rxDataPin)==0) { // JMC: 6.2.5 - Is the stop bit LOW? a LOW indicates an incorrect stop bit (inverted logic)
...
return; // JMC:
 ...
 _evenOrOdd = ((0x6996 >> _evenOrOdd) & 1); // JMC
  std::cout << "ReceiveChar() calculated parity of newChar (0 = even, 1 = odd): " << unsigned(_evenOrOdd) << "\n";

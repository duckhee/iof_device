...
module.exports = function(pool, socket, delivery) { //함수로 만들어 객체 app을 전달받음    
    return {
        init: function() {
            ...
            var shooting_time = 60; //사진 촬영 인터벌
        ...
        camera_shooting: function() {
            ...
            exec_photo(cmd_photo, function(err, stdout, stderr) {
                ...
                pool.getConnection(function(err, connection) {
                    ...
                    connection.query(' select * from seosan_settings  order by createdAt desc limit 0,1 ', function(err, result, fields) {
                        ...
                        if (result.length != 0 && result[0].st_serial) {
                            ...
                            //정보 insert
                            connection.query(' insert into seosan_images  (si_serial, si_path, si_filename, si_filesize, createdAt, updatedAt) values (?, ?, ?, ?, NOW(), NOW())', [result[0].st_serial, dir_name, timeInMs + ".jpg", stats.size], function(error, result) {
                                if (!error) {
                                    console.log(result);
                                }
                            });

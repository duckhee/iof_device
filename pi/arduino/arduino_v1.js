...
module.exports = function(pool, socket) {
    ...
    port.on('open', function() {
        ...
        var mesure_time = 30; //사진 센서 정보 가져오기 인터벌
        var sub_min = 0; //정각에서 남은 시간

        //인터벌 함수 실행
        if (current_min == 0) { // 만약 0이면 바로 수집 시작
            sub_min = 0;
        } else { // 0이 아닐시 남은 시간 설정 후 수집 시작
            sub_min = mesure_time - current_min;
        }
        ...
        setTimeout(() => {
            ...
            setInterval(port.write('ms'), 1000 * 60 * mesure_time); // 설정 시간 후에 반복 수집
        }, 1000 * 60 * sub_min); // 제한된 시간 후에 수집 시작

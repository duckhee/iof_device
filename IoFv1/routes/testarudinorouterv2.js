...
module.exports = function(pool, socket, serialNum, sensingtime) {
    return {
        init: function() {
            console.log('get setting camera time :::: ', sensingtime)
            var current_min = moment().format('m'); // 현재 시간 분 설정
            var sensingtime = sensingtime; //사진 촬영 인터벌
            ...
            if (current_min == 0) { // 만약 0이면 바로 촬영 시작
                ...
                sub_min = sensingtime - current_min;
            }
            ...
            setTimeout(() => {
                ...
                setInterval(this.sensing, 1000 * 60 * sensingtime); // 설정 시간 후에 반복 촬영

...
module.exports = function(pool, socket) {
    ...
    parser.on('data', function(data) {
        ...
        */
        console.log(data);
        var dataJoin = data;
        var sensorObj = data.split(","); // json 형식 data를 객체형식으로 저장
        ...
        if (sensorObj.length == 26 || sensorObj.length == 14) {
            ...
            socket.on('device_setting_receive_' + serial_num, function(data) {
                ...
                pool.getConnection(function(err, connection) {
                    //정보가 없을 시 정보 insert
                    connection.query(' update iof_settings set st_title =  ?, st_gps = ?, st_group = ?, st_ping = 1, updatedAt = NOW() where st_serial = ? ', [data.st_title, data.st_gps, data.st_group, data.st_serial], function(error, result) {
                        if (!error) {
            ...
            pool.getConnection(function(err, connection) {
                // Use the connection
                connection.query(' select * from iof_settings  where st_serial = ? ', [serial_num], function(err, result, fields) {
                    ...
                    if (result.length == 0) {
                        //정보가 없을 시 정보 insert
                        connection.query(' insert into iof_settings  (st_address, st_serial) values (?, ?)', [address, serial_num], function(error, result) {
            ...
            pool.getConnection(function(err, connection) {
                // Use the connection
                connection.query(' insert into iof_data  (sd_address, sd_serial, sd_data) values (?, ?, ?)', [address, serial_num, dataJoin], function(error, result) {
                    if (error) console.log(error);
                    if (!error) {
                        //console.log(result);
                        var sensor_obj = {
                        ...
                        socket.on('sensor_data_receive_' + serial_num, function(data) {
                            ...
                            if (data.msg == "1") {
                                ...
                                pool.getConnection(function(err, connection) {
                                    // Use the connection
                                    connection.query(' select * from iof_networks where sn_serial = ? ', [serial_num], function(err, result, fields) {
                                        if (err) console.log(err);

                                        if (result.length == 0) {
                                            //정보가 없을 시 정보 insert
                                            connection.query(' insert into iof_networks  (sn_address, sn_serial, sn_type, createdAt) values ( ?, ?, ?, NOW() )', [address, serial_num, 'ard'], function(error, result) {
                                                if (error) console.log(error);
                                                if (!error) {
                                        ...
                                        if (result.length > 0) {
                                            //정보가 있을시 update
                                            connection.query(' update iof_networks  set createdAt = NOW() where sn_serial = ? ', [serial_num], function(error, result) {
                                                if (error) console.log(error);
                                                if (!error) {
            ...
            try {
                ...
                fs.appendFileSync(process.cwd() + '/data/' + dir_name + '/' + file_name, address + "," + serial_num + "," + dataJoin + '\n', 'utf8');
            } catch (err) {
                console.log(err);
            }
